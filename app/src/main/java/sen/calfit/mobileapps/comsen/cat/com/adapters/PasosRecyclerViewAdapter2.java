...
import sen.calfit.mobileapps.comsen.cat.com.activities.ReceptaActivity2;
...
public class PasosRecyclerViewAdapter2 extends RecyclerView.Adapter<PasosRecyclerViewAdapter2.PasViewHolder> {
    private ArrayList<Pas> passos;
    private ReceptaActivity2 receptaActivity2;

    public PasosRecyclerViewAdapter2 (ArrayList<Pas> passos, ReceptaActivity2 receptaActivity2) {
        this.passos = passos;
        this.receptaActivity2 = receptaActivity2;
    }
    ...
    @NonNull
    @Override
    public PasosRecyclerViewAdapter2.PasViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.pasos2_item_layout, parent, false);
        PasosRecyclerViewAdapter2.PasViewHolder vh = new PasosRecyclerViewAdapter2.PasViewHolder(v);
        return vh;
    }

    @Override
    public void onBindViewHolder(final PasosRecyclerViewAdapter2.PasViewHolder holder, @SuppressLint("RecyclerView") final int position) {
        ...
        if(passos.get(position).getVideo()!=null){
            ...
            holder.playVideo.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    intent.putExtra("video", passos.get(position).getVideo());
                    receptaActivity2.startActivity(intent);
                }
            });
        }
        ...
        if(passos.get(position).getCounter()!=0){
            ...
            holder.buttonCountdown.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    receptaActivity2.sendNotification("Tiempo empezado del paso nº"+passos.get(position).getNumero());
                ...
                private void startTimer() {
                    mCountDownTimer[position] = new CountDownTimer(mTimeLeft[position], 1000) {
                        ...
                        public void onFinish() {
                            receptaActivity2.sendNotification("Tiempo terminado del paso nº"+passos.get(position).getNumero());
                        }
                    }.start();
